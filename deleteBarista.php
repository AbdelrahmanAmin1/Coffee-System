...
if (isset($_POST['delete']) && isset($_POST['name'])) { // Check if 'id' array is set
    foreach ($_POST['name'] as $x) {
        // Prepare and bind the SQL statement
        $stmt = $conn->prepare("DELETE FROM barista WHERE name = ?");
        $stmt->bind_param("s", $x);
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    if ($i % 2 == 1)
                        ...
                        echo "<tr > ";
                    echo "<td> <input type='checkbox' name='name[]' value='".$row['name']."'></td>"; // Checkbox for selecting
                    foreach ($row as $key => $value) {
